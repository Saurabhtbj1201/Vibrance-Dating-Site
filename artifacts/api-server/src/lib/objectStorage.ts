...
export class ObjectStorageService {
  ...
  normalizeObjectEntityPath(uploadURL: string): string {
    try {
      ...
      // Extract the 'uploads/...' portion and map it to our backend's public-objects route
      const match = url.pathname.match(/\/(uploads\/[^?]+)/);
      if (match) {
        return `/public-objects/${match[1]}`;
      }
      return uploadURL;
    } catch {
      return uploadURL;
  ...
  async searchPublicObject(filePath: string): Promise<string | null> {
    try {
      // If the path already has the uploads/ prefix, just return it directly
